...
class AddstudentController extends Controller
{
    public function xls_to_db(Request $request)
    {
        ...
        $matchThese1 = [
           ...
           try {
            User::updateOrCreate($matchThese1,$MarksSetup_data);
        ...
        for ($i=3; $i <count($request->data) ; $i++) {
            ...
            try {
                User::updateOrCreate($matchThese,$data_Marks);
                    // $MarksMark->save();
        ...
        public function updateall(Request $request)
        {
            try {
                ...
                foreach($request['todb'] as $row){
                    ...
                    $Grading=  User::findOrFail($row['id']); 
        ...
        public function deleteone(Request $request)
        {
            try {
                    ...
                    $Grading=  User::findOrFail($request['id']); 
                    $Grading->delete();
